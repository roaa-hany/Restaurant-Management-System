...
let currentKitchenSession: { id: string; name: string; role: string } | null = null;
...
namespace AuthHelpers {
    ...
    export function validateSession(expectedRole: 'waiter' | 'manager' | 'kitchen'): boolean {
        if (!currentKitchenSession) return false;
        if (currentKitchenSession.role !== expectedRole) return false;
        return true;
    }
    ...
    export function getCurrentUser(): { id: string; username: string; name: string; role: string } | null {
        return currentKitchenSession as any;
    }

    /**
     * Set session (called after login)
     */
    export function setSession(sessionData: any): void {
