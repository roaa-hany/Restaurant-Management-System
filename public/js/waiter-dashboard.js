...
function displayTables() {
    ...
    tablesContainer.innerHTML = waiterTables.map(table => {
        ...
        const tableOrders = waiterOrders.filter(order => order.tableNumber === table.number &&
...
function displayOrders() {
    ...
    const filteredOrders = waiterOrders.filter(order => (order.assignedWaiter === currentWaiter.id || !order.assignedWaiter) &&
        order.status !== 'paid');
...
function populateBillingOrderSelect() {
    ...
    // Only show orders that are served (ready for billing) - paid orders are excluded by the API
    const billableOrders = waiterOrders.filter(order => order.status === 'served');
...
async function processPayment() {
    ...
    try {
        ...
        if (response.ok) {
            // Add payment method to bill before printing
...
function generateAndPrintReceipt(bill) {
    ...
    try {
        ...
        if (receiptWindow) {
            ...
            // The print dialog will be triggered by the script in the receipt HTML
        }
        else {
...
function showTableModal(table) {
    if (!tableModal || !modalTableNumber || !tableDetails)
        return;
...
function calculatePerformanceMetrics() {
    ...
    const todaysOrders = waiterOrders.filter(order => new Date(order.createdAt).toDateString() === today);
    // Tables served today (tables with paid orders)
    const tablesServedToday = new Set(todaysOrders
        .filter(order => order.status === 'paid')
        .map(order => order.tableNumber)).size;
    ...
    if (activeTablesCount) {
        activeTablesCount.textContent = String(waiterTables.filter(table => table.status === 'occupied' || table.status === 'need-assistance').length);
    }
    if (pendingOrdersCount) {
        pendingOrdersCount.textContent = String(waiterOrders.filter(order => order.status === 'pending' || order.status === 'preparing').length);
    }
    if (todayRevenue) {
